...
fn main() {
    ...
    println!(
        "Inputs  length: {}, Inputs[0]  length: {}",
        testing_inputs.len(),
        testing_inputs[0].len()
    );
    println!(
        "Outputs length: {}, Outputs[0] length: {}",
        testing_outputs.len(),
        testing_outputs[0].len()
    );
    ...
    loop {
        for e in 0..(training_inputs.len() - 1) {
            let expected_output_vector = &training_outputs[e].to_bitvec();
            {
                {
                    let input_vector = &training_inputs[e];
                    let correct = check_two_vectors(
                        expected_output_vector,
                        tm.activate(input_vector.clone()),
                    );
                    average_training_error =
                        0.99 * average_training_error + 0.01 * (if !correct { 1.0 } else { 0.0 });
                }
            ...
            {
                {
                    ...
                    average_testing_error =
                        0.99 * average_testing_error + 0.01 * (if !correct { 1.0 } else { 0.0 });
                }
...
type BooleanMatrix = Vec<BitVec>;

fn read(file_path: &str) -> (BooleanMatrix, BooleanMatrix) {
    fn one_hot_encoder(n: u16) -> BitVec {
        assert!(n < 10);
    ...
    for result in rdr.records() {
        ...
        let vector: Vec<u16> = record.iter().map(|x| x.parse::<u16>().unwrap()).collect();

        {
            ...
            {
                let lwv = LabelWithData { label, data };
